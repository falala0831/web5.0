...
import com.lxl.web5.service.Tablealter;
...
public class Add {
    ...
    // 必须要先登录
    @RequestMapping("/add")
    public String showAdd(MessageInfor cont, Model model, HttpServletRequest request) {
    ...
    public String checkAdd(MessageInfor cont, HttpServletRequest request, Model model) {
        ...
        List<MessageInfor> list = userJpaRepository.findByContactname(cont.getContactname());
    ...
    public int checkTel(@RequestParam(name="tel")String tel, HttpServletRequest request) {
        ...
        boolean result = Tablealter.hasTel(userJpaRepository.findAll(), tel);
        if (result)
            return 1;
        else
            return 0;
